...
class HttpClient {
    constructor(opts) {
        ...
        const { baseURL, authToken, authType, memoryCache, diskCache, name, metrics, product, serverTiming, recorder, userAgent, timeout = DEFAULT_TIMEOUT_MS, segmentToken, sessionToken, retries, concurrency, headers: defaultHeaders, params, operationId, verbose, } = opts;
        this.name = name || baseURL || 'unknown';
        const limit = concurrency && concurrency > 0 && p_limit_1.default(concurrency) || undefined;
        const headers = Object.assign({}, defaultHeaders, { 'Accept-Encoding': 'gzip', 'User-Agent': userAgent }, operationId ? { 'x-vtex-operation-id': operationId } : null, segmentToken ? { [constants_1.SEGMENT_HEADER]: segmentToken } : null, sessionToken ? { [constants_1.SESSION_HEADER]: sessionToken } : null, product ? { [constants_1.PRODUCT_HEADER]: product } : null);
